...
- PRs target `develop`, the integration branch; `main` is release-only (releases are cut by pushing a `v*` tag). Include tests for behaviour changes and a README update when the public API grows.
