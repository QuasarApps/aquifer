...
dependencies {
    ...
    // OkHttp types (Call.Factory, Request, ResponseBody) appear in the public API.
    api(libs.okhttp)
...
mavenPublishing {
    ...
    signAllPublications()
    coordinates("io.github.quasarapps", "aquifer-okhttp", version.toString())
    configure(KotlinJvm(javadocJar = JavadocJar.Dokka("dokkaGeneratePublicationHtml"), sourcesJar = true))

    pom {
        name.set("Aquifer OkHttp")
        description.set("OkHttp conditional-fetching helper (ETag/Last-Modified revalidation) for Aquifer.")
